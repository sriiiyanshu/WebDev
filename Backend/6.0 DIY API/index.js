...
app.get("/random", (req, res)=>{
  ...
  if(!joke) return res.status(400).json({error: "joke not found"});
...
app.get("/joke", (req, res)=>{
  ...
  const joke = jokes.find(j => j.jokeType.toLowerCase() === type);
...
app.put("/joke/:id", (req, res)=>{
  const id = (Number)(req.params.id);
  const joke = jokes.find(j => j.id===id);
  if(!joke) return res.status(400).json({error: "post not found"});
...
app.patch("/joke/:id", (req, res)=>{
  ...
  joke.jokeText = req.body.joke || joke.jokeText;
...
app.delete("/joke/:id", (req, res)=>{
  jokes = jokes.filter(j=>j.id!==(Number)(req.params.id))
...
app.delete("/deleteAll", (req, res)=>{
  ...
  if(key === masterKey){
    ...
    res.status(400).json({error: "Not premitted"});
  }
})
